...
async function run() {
  ...
  try {
    ...
    // 1. Trigger Backtest
    const startRes = await fetch('http://127.0.0.1:3000/api/backtest/run', {
      method: 'POST',
    ...
    while (isRunning) {
      ...
      const progRes = await fetch(`http://127.0.0.1:3000/api/backtest/progress/${encodeURIComponent(runId)}`);
      if (progRes.ok) {
